...
@Component({
    ...
    standalone: true,
    imports: [CommonModule, RouterModule, AddToMealPlanModalComponent, FooterComponent, HeaderComponent]
})
export class FavouritesComponent implements OnInit {
  ...
  addRecipeToMealPlan(event: { recipe: Recipe, day: string, mealType: string }) {
    this.mealPlanService.addRecipeToMealPlan(event.recipe.id, event.day, event.mealType).subscribe(
      (response) => {
